...
public class ItemPrintout extends Item
{
    ...
    private static final String TAG_LINE_TEXT = "text_";
    private static final String TAG_LINE_COLOUR = "colour_";
    ...
    public ItemPrintout( Builder settings, Type type )
    {
        super( settings );
        this.type = type;
    }
    ...
    public void addInformation( @Nonnull ItemStack stack, World world, List<ITextComponent> list, ITooltipFlag options )
    {
        ...
        if( title != null && title.length() > 0 ) list.add( new TextComponentString( title ) );
    }
    ...
    private ItemStack createFromTitleAndText( String title, String[] text, String[] colours )
    {
        ...
        // Build NBT
        if( title != null ) stack.getOrCreateTag().putString( TAG_TITLE, title );
        if( text != null )
        {
            NBTTagCompound tag = stack.getOrCreateTag();
            tag.putInt( TAG_PAGES, text.length / LINES_PER_PAGE );
            for( int i = 0; i < text.length; i++ )
            {
                if( text[i] != null ) tag.putString( TAG_LINE_TEXT + i, text[i] );
            }
        }
        if( colours != null )
        {
            ...
            {
                if( colours[i] != null ) tag.putString( TAG_LINE_COLOUR + i, colours[i] );
            }
        }
    ...
    public static String getTitle( @Nonnull ItemStack stack )
    {
        ...
        return nbt != null && nbt.contains( TAG_TITLE ) ? nbt.getString( TAG_TITLE ) : null;
    }

    public static int getPageCount( @Nonnull ItemStack stack )
    {
        NBTTagCompound nbt = stack.getTag();
        return nbt != null && nbt.contains( TAG_PAGES ) ? nbt.getInt( TAG_PAGES ) : 1;
    }

    public static String[] getText( @Nonnull ItemStack stack )
    {
        return getLines( stack, TAG_LINE_TEXT );
    }

    public static String[] getColours( @Nonnull ItemStack stack )
    {
        return getLines( stack, TAG_LINE_COLOUR );
    }
