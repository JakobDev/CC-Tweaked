...
public interface ContainerType<T extends Container>
{
    ...
    {
        GuiProviderRegistry.INSTANCE.registerFactory( containerType.get().getId(), ( id, type, player, packet ) -> {
            T container = containerType.get();
            container.fromBytes( packet );
            return factory.apply( id, container, player );
        } );
    ...
    {
        GuiProviderRegistry.INSTANCE.<C>registerFactory( containerType.get().getId(), container ->
