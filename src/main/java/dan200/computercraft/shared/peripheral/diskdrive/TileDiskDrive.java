...
public class TileDiskDrive extends TileGeneric implements DefaultInventory, Tickable, IPeripheralTile
{
    ...
    {
        super.fromTag( nbt );
        if( nbt.containsKey( TAG_ITEM ) )
        {
            CompoundTag item = nbt.getCompound( TAG_ITEM );
    ...
    private synchronized void ejectContents( boolean destroyed )
    {
        ...
        if( !m_diskStack.isEmpty() )
        {
            ...
            entityitem.velocityX = xOff * 0.15;
            entityitem.velocityY = 0.0;
            entityitem.velocityZ = zOff * 0.15;

            getWorld().spawnEntity( entityitem );
            if( !destroyed )
            {
                getWorld().fireWorldEvent( 1000, getPos(), 0 ); // BLOCK_DISPENSER_DISPENSE (See Renderer)
            }
        }
    }
    ...
    public final void readDescription( @Nonnull CompoundTag nbt )
    {
        ...
        m_diskStack = nbt.containsKey( TAG_ITEM ) ? ItemStack.fromTag( nbt.getCompound( TAG_ITEM ) ) : ItemStack.EMPTY;
        updateBlock();
    }
