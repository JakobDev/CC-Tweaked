...
public class PocketServerComputer extends ServerComputer implements IPocketAccess
{
    ...
    public int getLight()
    {
        ...
        return tag.contains( "light", Constants.NBT.TAG_ANY_NUMERIC ) ? tag.getInt( "light" ) : -1;
    }
    ...
    public void setLight( int colour )
    {
        ...
        {
            if( !tag.contains( "light", Constants.NBT.TAG_ANY_NUMERIC ) || tag.getInt( "light" ) != colour )
            {
                tag.putInt( "light", colour );
                updateUserData();
            }
        }
        else if( tag.contains( "light", Constants.NBT.TAG_ANY_NUMERIC ) )
        {
            tag.remove( "light" );
            updateUserData();
        }
