...
public class ItemPocketComputer extends Item implements IComputerItem, IMedia, IColouredItem
{
    ...
    public ItemPocketComputer( Builder settings, ComputerFamily family )
    {
        super( settings );
        this.family = family;
    ...
    private static int getInstanceID( @Nonnull ItemStack stack )
    {
        ...
        return compound != null && compound.contains( TAG_INSTANCE ) ? compound.getInt( TAG_INSTANCE ) : -1;
    }
    ...
    {
        NBTTagCompound compound = stack.getTag();
        return compound != null && compound.contains( TAG_SESSION ) ? compound.getInt( TAG_SESSION ) : -1;
    }
    ...
    public static int getLightState( @Nonnull ItemStack stack )
    {
        ...
        if( computer != null && computer.isOn() )
        {
            ...
            if( computerNBT != null && computerNBT.contains( TAG_LIGHT ) )
            {
    ...
    {
        NBTTagCompound compound = stack.getTag();
        return compound != null && compound.contains( TAG_UPGRADE )
    ...
    {
        return stack.getOrCreateChildTag( TAG_UPGRADE_INFO );
    }
